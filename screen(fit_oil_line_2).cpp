...
void leftSide(bool write_flag)
{
	...
	if (write)
	{
		lineResult = drawLine(lineParam1, lineResult, 0, 1);
		lineResult = drawLine(lineParam2, lineResult, 1, 1);
		lineResult = drawLine(lineParam3, lineResult, 0, 1);
	...
	threshold(thresOil, thresOil, (flag[1] + flag[0]) / 4 , 255, THRESH_BINARY);
	if (write)
		imwrite("./test2/thresOil.jpg", thresOil);
	// calculate shift
	int shiftAmount;
	// top
	shiftAmount = calShift(thresOil, 100, thresOil.cols - 100, roi1.tl().y - 1000, roi1.tl().y + 2500, 1, write);
	lineParam1[3] -= shiftAmount; 
	// middle
	shiftAmount = calShift(thresOil, 100, roi2_m.br().y, roi2_m.tl().x - 500, roi2_m.tl().x + 2000, 2, write);
	lineParam2[2] -= shiftAmount;  
	// bottom
	shiftAmount = calShift(thresOil, 100, roi3.tl().x, roi3.tl().y + 500, roi3.tl().y + 2000, 3, write);
...
{
	Mat imgR1 = imread("./Origin2/2/1.jpg", IMREAD_GRAYSCALE);
	...
	threshold(thresOil, thresOil, flag[0], 255, THRESH_TOZERO);
	//threshold(thresOil, thresOil, (flag[1] + flag[0]) / 4, 255, THRESH_BINARY);
	//threshold(thresOil, thresOil, 1, 255, THRESH_BINARY);
	if (write)
		imwrite("./test2/thresOil.jpg", thresOil);
	// calculate shift
	int shiftAmount;
	// top
	shiftAmount = calShift(thresOil, 100, 0, roi1.tl().y - 1000, roi1.tl().y + 2500, 1, write);
	lineParam1[3] -= shiftAmount;
	// middle
	Mat sub = imgR2 - imgR1;
	Mat thresOil2;
	drawHist(&sub, write, showHist);
	threshold(sub, thresOil2, flag[1], 255, THRESH_TOZERO_INV);
	threshold(thresOil2, thresOil2, flag[0], 255, THRESH_TOZERO);
	shiftAmount = calShift(thresOil2, 100, roi2_m.br().y - 100, roi2_m.tl().x, roi2_m.tl().x + 2000, 2, write);  //726
	lineParam2[2] += shiftAmount;
	// bottom
	//shiftAmount = calShift(thresOil, 100, roi3.br().x, roi3_m.tl().y + 500, roi3.tl().y + 2000, 3, true);
	shiftAmount = calShift(thresOil, 100, 0, roi3_m.tl().y + 500, roi3.tl().y + 2000, 3, write); 
	lineParam3[3] += shiftAmount;
	if (write)
	{
		lineResult = drawLine(lineParam1, lineResult, 0, 1);
...
void middleSide(bool write_flag)
{
	...
	lineParam1[3] -= shiftAmount;
	// bottom
	shiftAmount = calShift(thresOil, 100, 0, roi3.tl().y, roi3.tl().y + 2500, 1, write);
	lineParam3[3] += shiftAmount;
	if (write)
	{
		lineResult = drawLine(lineParam1, lineResult, 0, 1);
